...
## Workflow
1.  **Initialize**: Create the repository and basic folder structure.
2.  **Design**: Write `SPECS.md` or `architecture.pml`.
3.  **Define**: Create `src/core/interfaces.py` (or similar) with abstract base classes.
4.  **Delegate**: Use the `create_work_item` skill to assign tasks.
    *   *Input*: Context files (the interfaces you just wrote).
    *   *Output*: Target files (where the Contributor should write code).

## Tools / Skills
*   `define_interface(path, content)`: Use this to save your designs.
*   `create_work_item(...)`: Use this to assign work.
